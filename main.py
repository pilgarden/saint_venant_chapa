...
def aplicar_tema(fig, axes, bg_color, fg_color):
    """Aplica o tema (claro/escuro) aos elementos do Matplotlib."""
...
def plot_chapa(L, H, raio, tensao, bg_color='white', fg_color='black'):
    """Gera o esquema visual do problema de contorno."""
    fig, ax = plt.subplots(figsize=(8, 4), dpi=100)
    
    # Desenho da chapa
    chapa = patches.Rectangle((0, 0), L, H, lw=2, ec=fg_color, fc=bg_color, zorder=1)
    ax.add_patch(chapa)
    
    # Desenho do Furo
    furo = patches.Circle((L/2, H/2), raio, fc=bg_color, ec=fg_color, lw=1.5, zorder=2)
    ax.add_patch(furo)
    
    # Hachuras do engaste
    num_hachuras = 15
    for i in np.linspace(0, H, num_hachuras):
        ax.plot([-L*0.04, 0], [i, i-H*0.04], color=fg_color, lw=1)
    ax.plot([0, 0], [0, H], color=fg_color, lw=3)
    ax.text(-L*0.06, H/2, 'Engaste Fixo\n(u=0, v=0)', ha='right', va='center', fontweight='bold', color=fg_color)
    ...
    # Cotas de Dimensões
    ax.annotate('', xy=(0, -H*0.08), xytext=(L, -H*0.08), arrowprops=dict(arrowstyle='<->', color=fg_color))
    ax.text(L/2, -H*0.15, f'L = {L}mm', ha='center', color=fg_color)
    ax.annotate('', xy=(L*0.1, 0), xytext=(L*0.1, H), arrowprops=dict(arrowstyle='<->', color=fg_color))
    ax.text(L*0.06, H/2, f'H = {H}mm', rotation=90, va='center', color=fg_color)
    ...
    aplicar_tema(fig, ax, bg_color, fg_color)
    plt.tight_layout()
    return fig
...
def plot_mapa_calor(X, Y, s_x, Xc, Yc, L, H, raio_furo, tensao_axial, x_coords, bg_color, fg_color):
    ...
    fig, ax_map = plt.subplots(figsize=(8, 4.5), dpi=100)
    
    cp = ax_map.contourf(X, Y, s_x, levels=120, cmap='jet', vmin=vmin_plot, vmax=vmax_plot)
    cbar = fig.colorbar(cp, ax=ax_map)
    cbar.set_label('Tensão Longitudinal $\sigma_{xx}$ (MPa)', fontsize=12, color=fg_color)
    cbar.ax.yaxis.set_tick_params(color=fg_color, labelcolor=fg_color)
    
    furo = plt.Circle((Xc, Yc), raio_furo, color=bg_color, zorder=12, ec=fg_color, lw=1.2)
    ...
    for idx, x_val in enumerate(x_coords):
        ...
        bbox_props = dict(boxstyle="round,pad=0.3", fc=bg_color, ec=cor, lw=1.5, alpha=0.9)
        ax_map.text(x_val + L*0.005, H*0.05, f'x={x_val:g}', color=fg_color, fontsize=10, fontweight='bold', bbox=bbox_props)

    ax_map.set_title('Campo de Tensões $\sigma_{xx}$ (Equações de Kirsch)', fontsize=14, fontweight='bold')
    ax_map.set_xlabel('Comprimento (mm)', fontsize=11)
    ax_map.set_ylabel('Altura (mm)', fontsize=11)
    ax_map.set_aspect('equal')
    
    aplicar_tema(fig, ax_map, bg_color, fg_color)
    plt.tight_layout()
    return fig

def plot_graficos_secao(X, Y, s_x, Xc, L, H, tensao_axial, comparativo, x_coords, bg_color, fg_color):
    ...
    if comparativo:
        ...
        fig, axes_sections = plt.subplots(1, n_sec, figsize=(max(12, 3.5 * n_sec), 4.5), dpi=100)
        if n_sec == 1:
    ...
    for i, (sx, color) in enumerate(zip(x_coords, cmap_colors)):
        ...
        if not comparativo:
            target_ax.set_title(lab, fontsize=12, color=color, fontweight='bold')
            target_ax.set_xlim(vmin_plot * 1.1, vmax_plot * 1.1)
            target_ax.axvline(0, color=fg_color, lw=1, ls='-')
        
        target_ax.axvline(tensao_axial, color='gray', ls='--', alpha=0.7, lw=1.5, label='Tensão Nominal' if i==0 else "")
        target_ax.grid(True, linestyle=':', alpha=0.4, color=fg_color)
        target_ax.set_xlabel('$\sigma_{xx}$ (MPa)', fontsize=11)
        target_ax.tick_params(labelsize=10)

    axes_sections[0].set_ylabel('Altura da Chapa $y$ (mm)', fontsize=11)
    if comparativo: 
        legend = axes_sections[0].legend(fontsize=11, loc='best')
        plt.setp(legend.get_texts(), color=fg_color)
        legend.get_frame().set_facecolor(bg_color)
        legend.get_frame().set_edgecolor(fg_color)

    aplicar_tema(fig, axes_sections, bg_color, fg_color)
    plt.tight_layout()
    return fig
...
def main():
    ...
    with st.sidebar:
        ...
        if "Claro" in tema_selecionado:
            ...
            bg_color, fg_color = "#0E1117", "white" # Cores padrão do Streamlit Dark Theme
    ...
    if geometria_valida:
        ...
        with col_esq:
            ...
            fig_esquema = plot_chapa(L, H, raio, tensao, bg_color, fg_color)
        ...
        with col_dir:
            ...
            with st.spinner("Computando malha..."):
                ...
                fig_mapa = plot_mapa_calor(X, Y, s_x, Xc, Yc, L, H, raio, tensao, x_coords, bg_color, fg_color)
                st.pyplot(fig_mapa)

        # SEGUNDA LINHA: Gráficos de Seção Isolados
        st.divider()
        st.subheader("Cortes Transversais do Perfil de Tensão")
        fig_secoes = plot_graficos_secao(X, Y, s_x, Xc, L, H, tensao, graficos_juntos, x_coords, bg_color, fg_color)
        st.pyplot(fig_secoes)
